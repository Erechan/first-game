...
func _ready() -> void:
	...
	# 关键修改：设置摇杆的鼠标过滤为Pass，允许输入穿透
	mouse_filter = Control.MOUSE_FILTER_PASS
	if _base:
		_base.mouse_filter = Control.MOUSE_FILTER_PASS
	if _tip:
		_tip.mouse_filter = Control.MOUSE_FILTER_PASS

func _input(event: InputEvent) -> void:
	if event is InputEventScreenTouch:
		if event.pressed:
			# 只有触摸在摇杆区域内且未占用触摸ID时，才处理摇杆逻辑
			if _is_point_inside_joystick_area(event.position) and _touch_index == -1:
				if joystick_mode == Joystick_mode.DYNAMIC or joystick_mode == Joystick_mode.FOLLOWING or (joystick_mode == Joystick_mode.FIXED and _is_point_inside_base(event.position)):
					...
					_is_joystick_touch = true
					# 仅处理摇杆触摸时，标记输入已处理
					get_viewport().set_input_as_handled()
				else:
					# 非摇杆核心区域的触摸，放行给按钮
					_is_joystick_touch = false
			else:
				# 摇杆区域外的触摸，直接放行
				_is_joystick_touch = false
